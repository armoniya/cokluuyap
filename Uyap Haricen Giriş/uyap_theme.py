# -*- coding: utf-8 -*-
"""
Çoklu UYAP masaüstü — ortak tema (Panel/theme.py'nin uygulamaya taşınmış kopyası)
=================================================================================
Projenin TEK tasarım dili Panel/theme.py'de tanımlıdır: ılık kâğıt zemin + adaçayı
vurgu. Bu uygulama ayrı paketlendiği (PyInstaller) için palet buraya kopyalandı.

KURAL: Panel/theme.py'de palet/şekil değişirse BU dosya da birlikte güncellenir.
Buradaki tek ek, RoundButton.configure()'ün ttk.Button çağrılarıyla (state=/text=)
uyumlu olmasıdır; gerisi birebir kopyadır.
"""
...
class RoundButton(tk.Canvas):
    ...
    def configure(self, cnf=None, **kw):
        """ttk.Button ile yer değiştirebilsin diye: state=/text=/command= anahtarları
        buton API'sine çevrilir, kalanı tuvale geçer (uyap_app'e özel ek)."""
        ...
        if "command" in kw:
            self.set_command(kw.pop("command"))
        if kw:
...
def make_fonts():
    """Kabuğun kullandığı yazı tiplerini üretir (Panel ile aynı ölçek)."""
    return {
